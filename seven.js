...
// var ethnicColor = d3.scaleThreshold().domain([0, 500, 1000, 10000, 50000, 100000, 500000]).range(['#C6DBEF', '#9ECAE1', '#6baed6', '#3399cc', '#005ea8', '#3b508b', '#081d58']);
// var color_14 = d3.scaleThreshold().domain([0, 500, 1000, 10000, 50000, 100000, 500000]).range(['#f0eff9', '#d7d5ef', '#ccc9ea', '#b9b5e0', '#908dc6', '#918fc4', '#807dba']);
// d3.select("#btn-zoom--in").on("click", () => clickToZoom(ZOOM_IN_STEP));
// d3.select("#btn-zoom--out").on("click", () => clickToZoom(ZOOM_OUT_STEP));
var ethnicColor = d3.scaleThreshold().domain([0, 1000, 10000, 50000, 100000, 500000]).range(['rgb(34, 47, 62, 0.05)', 'rgb(34, 47, 62, 0.1)', 'rgb(34, 47, 62, 0.2)', 'rgb(34, 47, 62, 0.38)', 'rgb(34, 47, 62, 0.5)', 'rgb(34, 47, 62, 0.75)']);
var color_14 = d3.scaleThreshold().domain([0, 1000, 10000, 50000, 100000, 500000]).range(['rgb(238, 82, 83, 0.05)', 'rgb(238, 82, 83, 0.1)', 'rgb(238, 82, 83, 0.25)', 'rgb(238, 82, 83, 0.45)', 'rgb(238, 82, 83, 0.68)', 'rgb(238, 82, 83, 1)']);
...
var g = svg.append("g").attr('id', 'maploc')
...
function renderMap(root, update, type) {
    if (type == 'age') {
        ageG.append("g")
            .selectAll("path")
            .data(root.features)
            ...
            .attr("fill", function (d, i) {
                if (update) {
                    return ethnicColor(d.properties.density)
                } else {
                    return "#A9A9A9"
                }
            })
            ...
            .on("mouseout", function (d, i) {
                if (d.properties.density !== undefined && update) {
    ...
    if (type == 'ethnic') {
        ...
        g
            ...
            .attr("fill", function (d, i) {
                if (update) {
                    console.log(ethnicColor(d.properties.density), d.properties.density, d.properties.name)
...
function drawLegend(type, threshold) {
    ...
    function column(title, scale) {
        ...
        if (type == 'ethnic') {
            var legendsvg = div.insert("div", "#map__container + *").attr("class", "col-2 legend" + type).text("Population")
                .style("padding", "0px").append("svg").attr("height", "100%").attr("width", "200px");
        }
        if (type == 'age') {
            var legendsvg = div.insert("div", "#age_map__container + *").attr("class", "col-2 legend" + type).text("Population")
                .style("padding", "0px").append("svg").attr("height", "100%").attr("width", "200px");
        }
        // div.append("span")
...
function updateMap(rental_type = 'none') {
    const rental_types = {
        1: "Bachelor",
        2: "1-Bedroom",
        3: "2-Bedroom",
