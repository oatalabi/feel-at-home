...
g
    .attr(
        "transform", 'translate(130,40) scale(0.2)'
    )
...
function updateMap(type='none') {
    ...
    if(type == 'default'){
        console.log('default selected now')
        ...
        if (type != 'none') {
            console.log('bedroom type selected now, =====')
            rental_type = type
            rentalData.forEach(function (d) {
                if (d["name"] != "All") {
            ...
            rental_type = rental_types[document.getElementById("rental-type")]
            if (rental_type != undefined){
                rent_val = rental_type.val
                ...
                // rentalData.forEach(function (d) {
                //     d["areas"].forEach(function(i){
                //         if (i["price"] <= parseInt(slider_price)) {
                //             new_loc.push(i)
                //         }
                //     })
                ...
                new_loc = rentalData.forEach(function (d) {
                    if (d["name"] != "All") {
                        d["areas"].forEach(function (i) {
...
function drawLegend(type, threshold) {
    ...
    function column(title, scale) {
        ...
        if(type == 'age'){
            var legendsvg = div.insert("div", "#age_map__container + *").attr("class", "col-2 legend"+type)
                .style("padding", "0px").append("svg").attr("height", "100%").attr("width", "200px");
        }
